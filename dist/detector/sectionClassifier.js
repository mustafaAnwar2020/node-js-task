import { sectionSchema } from "../contracts/themeSchema.js";
function hasAny(text, needles) {
    const lc = text.toLowerCase();
    return needles.some((needle) => lc.includes(needle));
}
function estimateType(tag, text, className, height, width) {
    const signals = [];
    let type = "unknown";
    let confidence = 0.45;
